...
FROM transmisiones t
        JOIN contenidos c ON t.id_contenido = c.id_contenido
        JOIN peliculas p ON p.id_pelicula = c.id_pelicula
WHERE t.alias_usuario = 'lucky' 
UNION 
SELECT s.titulo AS Titulo
FROM transmisiones t
        JOIN contenidos c ON t.id_contenido = c.id_contenido
...
SELECT 'Consulta 5';
SELECT DISTINCT t.alias_usuario, u.nombre, u.apellido
FROM transmisiones t
        INNER JOIN contenidos c ON c.id_contenido = t.id_contenido
        AND c.id_contenido IN 
				(SELECT id_pelicula FROM peliculas WHERE titulo = 'Interestelar')
		INNER JOIN usuarios u ON u.alias = t.alias_usuario
...
SELECT * FROM contenidos c, peliculas p WHERE c.id_pelicula = p.id_pelicula;